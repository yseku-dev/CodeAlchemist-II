 ...
 * Includes a function to fetch available models from Groq.
 */
...
/**
 * Server Action to fetch available models from the Groq API using the groq-sdk.
 *
...
export async function getGroqModels(apiKey: string): Promise<GetGroqModelsResult> {
  console.log(`[SERVER ACTION getGroqModels] Iniciada. API Key (primeros 5 chars): ${apiKey ? apiKey.substring(0, 5) : 'NINGUNA'}...`);
  ...
  try {
    ...
    console.log("[SERVER ACTION getGroqModels] Respuesta cruda de groq.models.list() recibida:", JSON.stringify(chatModels, null, 2));
    ...
    let friendlyErrorMessage = "Error desconocido al contactar la API de Groq a través del SDK.";
    let errorDetails: any = { message: "Unknown error object" };

    if (error instanceof Error) {
        friendlyErrorMessage = `Error del SDK de Groq: ${error.name} - ${error.message.substring(0,150)}${error.message.length > 150 ? '...' : ''}`;
        errorDetails = { name: error.name, message: error.message, stack: error.stack?.substring(0, 200) };
        // Check for common Groq APIError properties if available, though error might not be a Groq.APIError directly
        if ('status' in error && typeof error.status === 'number') {
            if (error.status === 401) friendlyErrorMessage = "Error de autenticación con Groq: Clave API inválida o sin permisos.";
            else if (error.status === 429) friendlyErrorMessage = "Límite de tasa alcanzado con la API de Groq. Inténtalo más tarde.";
            else friendlyErrorMessage = `Error de la API de Groq (Status ${error.status}): ${error.message.substring(0,100)}...`;
        }
    ...
    console.error(`[SERVER ACTION getGroqModels] Error general capturado: ${friendlyErrorMessage}`, error);
